...
# args[15] to specific haplotype dosage
output_HD <- as.logical(args[15])

# args[16] to phasing
phased_gt <- as.logical(args[16])
...
if(niterations <= 18){
     ...
     shuffleHaplotypeIterations = NA,
       ...
       output_haplotype_dosages = output_HD,
                     do_phasing = phased_gt,
...
system(paste0("rsync -avz ", rsd, "/stitch.*.vcf.gz ", outputdir))
